...
if (process.env.NODE_ENV === 'production') {
  Sentry.init({
    ...
    // Disable user feedback dialog to avoid DialogContent accessibility issues
    // User feedback can be implemented separately with proper accessibility
    integrations: function(integrations) {
      // Filter out UserFeedback integration that may use Radix UI DialogContent
      return integrations.filter(integration => integration.name !== 'UserFeedback');
    },
  });
} else {
