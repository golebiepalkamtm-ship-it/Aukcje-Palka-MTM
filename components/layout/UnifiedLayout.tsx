...
=======
	...
	return (
		<>
			...
			{showNavigation && (
				<div
					className="absolute z-[1001] pointer-events-auto fade-in-fwd top-5 left-12 right-6"
				...
				>
					<div className="flex items-center justify-between w-full" style={{ perspective: '1500px' }}>
						...
						<div className="flex items-center gap-6">
							{/* Logo */}
							<div
								className="flex items-center scale-110 animate__animated animate__slow animate__backInDown"
								style={{ animationDelay: '0s' }}
							>
								<LogoGlow />
							</div>
								...
								<div className="flex items-center gap-4">
									{navItems.map((item, index) => {
										...
										return (
											<div
												key={item.href}
												className={`fade-in-flip-3d flex items-center`}
												style={{
													animationDelay: `${delay.toFixed(2)}s`,
													transformStyle: 'preserve-3d',
												}}
											>
												...
												>
													<i className={`${item.icon} relative z-10 text-3xl`}></i>
													<span className="relative z-10 text-sm ml-2 font-medium">
														{item.label}
													</span>
												</Link>
											</div>
			...
			<div className={`relative z-20 flex-1 ${isHomePage ? 'pt-20' : 'pt-40'}`} suppressHydrationWarning>
				{children}
			</div>
