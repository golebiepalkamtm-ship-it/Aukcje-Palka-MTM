...
export function AuctionsPage() {
	...
	>('all');
	// const [viewMode, setViewMode] = useState<'grid' | 'list'>('list'); // Unused for now
	...
	const fetchAuctions = useCallback(async () => {
		...
		try {
			...
			if (response.ok) {
				...
				// Mapuj assets na images/documents dla kompatybilności
				const auctionsWithAssets = (data.auctions || []).map(
					(auction: {
				...
				setAuctions(auctionsWithAssets);
			} else {
			...
			<div className="pt-32 pb-8 px-4 sm:px-6 lg:px-8">
				...
				<div className="relative z-10 px-2 sm:px-4 lg:px-6 xl:px-8 2xl:px-12">
					<div className="w-full">
						...
						{/* Auctions List View (usunięto zewnetrzne, niepotrzebne achievement-card) */}
						<div className="p-4">
											...
											return (
												...
												>
													<Link href={`/auctions/${auction.id}`} className="block">
														{/* JEDYNY POPRAWNY WRAPPER DLA GLOW Z REF */}
																			...
																			<div className="pt-2" onClick={e => e.stopPropagation()}>
																				{auction.status === 'ENDED' ? (
																					...
																					<div className="space-y-2">
																						<button
																							// Naprawiony link do detali aukcji
